...
# Python     : Python 2.7
# -------------------------------------------------------
# NAME       : check_exist_unityverts
# Describe   : 说明描述
# version    : v0.01
# Author     : linhuan
# Email      : hanhong@papegames.net
# DateTime   : 2023/8/22__18:42
# -------------------------------------------------------
...
reload(meshcommon)
EXP = ['Cam', 'FX']
EXP_GRPS = ['NPC_*_split','*_split_*','*_split']
...
class Check(object):
    ...
    def checkinfo(self):
        ...
        vers_data = self.get_verts_data()
        print vers_data
        for i in range(len(vers_data)):
            ver_info.append(u"[{}] 组下模型顶点数为:[{}]".format(vers_data[i][0], vers_data[i][1]))
        ver_info.append(self.end)
        ...
        if _error:
            ...
            return True, info.displayInfo(title=self.tooltip, objList=ver_info)
        ...
        if str(self._asset_level) not in ['10']:
            if self.verts:
                for k, v in self.verts.items():
                    if k and v:
                        _meshs_verts = self._select_grps_verts(k)[-1]
    ...
    def get_verts_data(self):
        verts_data = []
        if self.verts:
            for k, v in self.verts.items():
                if k and v:
                    _meshs_verts, grps = self._select_grps_verts(k)
                    verts_data.append([_meshs_verts, grps])
        return verts_data
    ...
    def _select_grps_verts(self, _grp):
        ...
        grp_ = ''
        if _grps:
            grp_ = _grps[0].split('|')[-1]

        return grp_, _numverts
    ...
    def _select_meshs_verts(self, _meshs):
        ...
        if _meshs:
            for i in range(len(_meshs)):
                if i == 0:
                    _num = self._get_mesh_vernums(_meshs[i])

                else:
                    _num = _num + self._get_mesh_vernums(_meshs[i])
        return _num

    def _get_mesh_vernums(self, mesh):
        u"""
        获取模型顶点数
        :param mesh:
        :return:
