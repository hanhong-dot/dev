...
class ProcessImpFace(object):
    def __init__(self):
        super(ProcessImpFace, self).__init__()

    def process_imp_face(self):
        imp_meshs = self._get_imp_mesh()


        if not imp_meshs:
            return
        # __process_data=self.__process_copy_meshs(billboard_meshs)
        for mesh in imp_meshs:

            # billboard_face_data = self._get_pology_structur(mesh)

            import_face_data=self._get_import_face_data(mesh)
            if not import_face_data:
                continue
            # self._triangulate_meshs([mesh])
            for k, v in import_face_data.items():
                self._process_import_face(v)
        # self.__process_process_data(__process_data)
        self.__set_meshs_vertex_display(imp_meshs)
        self.__remove_history(imp_meshs)
    ...
    def _process_import_face(self, vertices, distance=0.00001):
    ...
    def _get_import_face_data(self, mesh):
    ...
    def _get_imp_mesh(self):
        ...
        for mesh in mesh_list:
            if cmds.listRelatives(mesh, p=1, type='transform', f=1):
                tr = cmds.listRelatives(mesh, p=1, type='transform', f=1)[0]
                tr_short = tr.split('|')[-1]
                if tr_short.endswith('_Imp'):
                    meshs.append(tr)
        return meshs


# if __name__ == '__main__':
#     handle = ProcessImpFace()
#     handle.process_imp_face()
