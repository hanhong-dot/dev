# -*- coding: utf-8 -*-
# -----------------------------------
# @File    : process_item_sub_republish.py
# @Author  : linhuan
# @Time    : 2025/10/5 11:35
# @Description : 自动上传item变体资产
...
class ItemRePubSub(object):
    ...
    def republish_sub_assets(self, open_file=True, sub_asset_types=['item']):
        file_name = BaseFile().current_file()
        sub_ok, sub_assets = self._get_sub_republish_assets(sub_asset_types)
        ...
        if sub_ok == True and sub_assets:
            ...
            for asset_id in sub_assets:
                ...
                # else:
                __work_file='{}.{}.v001.ma'.format(asset_name, 'drama_rig')
                __task_data = get_task.TaskInfo(__work_file, 'X3', 'maya', 'publish', is_lastversion=True)
                if not __task_data:
                    print u'{} {} not task, please check'.format(asset_name, 'drama_rig')
                    continue
                if __task_data:
                    __work_path=__task_data.work_path
                    BaseFile().save_file(__work_path)
                    ok, resutl = self._batch_publish(asset_name, 'drama_rig', __work_path, version_file=None)
                    if ok == True:
                        print u'{} {} publish successful, please check'.format(asset_name, 'drama_rig')
                        print __work_path
                        publish_dict[asset_name] = resutl
    ...
    def _get_sub_republish_assets(self, sub_asset_types):
