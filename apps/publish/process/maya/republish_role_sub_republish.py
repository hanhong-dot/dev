...
class RePubliSub(object):
    ...
    def republish_sub_assets(self, open_file=True,copy_publish_file=True):
        file_name = BaseFile().current_file()
        sub_ok, sub_assets = self._get_sub_republish_assets()
            ...
            for asset_id in sub_assets:
                ...
                if rbf_ok == False:
                    ...
                    if rig_ok == True:
                        ...
                        print u'{} {} does not exist or has no pbulish history, please check'.format(asset_name, 'drama_rig')
        if publish_dict:
    ...
    def _get_sub_republish_assets(self):
        if self.judge_sub_republish() != True:
            return False, u'no need to publish sub assets'
        sub_assets = self._get_asset_sub_assets_info()
        asset_ids = self.get_asse_ids_from_sub_data(sub_assets)
    ...
    def _get_asset_sub_assets_info(self):
        return sg_asset_fun.get_asset_sub_sub_asset_ids(self.__sg, self.__entity_id)
    ...
    def judge_sub_republish(self):
        ...
        if self.__task_name not in TASKS:
            ...
            result = False
        if self.__entity_name not in PROCESSASSETS:
            print('entity name error')
            result = False
        ...
        for name in SPNAMES:
            if '{num}' in name:
                for num in range(1, 1000):
                    name_new = name.replace('{num}', str(num).zfill(3))
                    if '{count}' in name_new:
                        for count in range(1, 100):
                            name_new = name_new.replace('{count}', str(count))
