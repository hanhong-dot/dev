...
class PPublishWidget(QFrame):
    def __init__(self, task_data, parent=None):
        super(PPublishWidget, self).__init__(parent)
    ...
    def _get_user(self):
        ...
        @return:
        """
        return self._task_data.current_user_name
    ...
    def _user_authentication(self):
        ...
        if (_cur_user and _assign and _cur_user in _assign) or _cur_user.lower() == 'linhuan':
            return True
        else:
            return False

    # publish按钮
    def do_publish(self, _version_src, _description, _statu='pub',send_jenkins=False):
        infolist = []
        self._mb_export = True
                        ...
                        if key == 'version':
                            for obj_dic in value:
                                infolist.append(_datapack.Pack.set_info(
                                    ...
                                    description=obj_dic[
                                        'description'] if 'description' in obj_dic.keys() else _description,
                                    status=obj_dic['status'] if 'status' in obj_dic.keys() else _statu,
    ...
    def do_publish_without_process(self, updata_dict, _description, _statu='pub'):

        infolist = []
        self.appendSignal = Append_Signal()

        if updata_dict:
            if isinstance(updata_dict, dict):
                for key, value in updata_dict.items():
                    if key == 'version':
                        for obj_dic in value:
                            infolist.append(_datapack.Pack.set_info(
                                src_path=obj_dic['src_path'] if 'src_path' in obj_dic.keys() else '',
                                des_path=obj_dic['des_path'] if 'des_path' in obj_dic.keys() else obj_dic[
                                                                                                      'des_dir'] + '/' + os.path.basename(
                                    ''),
                                description=obj_dic[
    ...
    def do_publish_without_version(self, _description, _statu='pub'):
        '''
        ...
        if self._analyse_data.get_publishdata()[3]:
            ...
            if sub_dict:
                if isinstance(sub_dict, dict):
                    for key, value in sub_dict.items():
                        if key in ['publish', 'fbx', 'mbfbx', 'mbexfbx','mocap','ue','unityfbx','unityxml']:
                            for obj_dic in value:
                                infolist.append(_datapack.Pack.set_info(
                                    src_path=obj_dic['src_path'],
                                    des_path=obj_dic['des_path'],
    ...
    def _do_processcmd(self, cmd):
        if cmd and isinstance(cmd, str):
            ...
            _command_new = _command.replace('TaskData_Class', 'self._task_data').replace('mb_export', 'self._mb_export').replace('version_file', 'self._version_file').replace('export_fbx_grp_list','all')
...
#     win = PublishWidget(get_task.TaskInfo('CutScene_ML_C10S1_S01_P01.cts_rough.v004.fbx', 'X3', 'motionbuilder', 'publish'))
