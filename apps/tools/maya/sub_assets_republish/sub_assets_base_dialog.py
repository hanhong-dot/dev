...
# Python     : Python 2.7
# -------------------------------------------------------
# NAME       : sub_assets_base_dialog
# Describe   : 基础面板
# version    : v0.01
# Author     : linhuan
# Email      : hanhong@papegames.net
# DateTime   : 2023/11/25__17:23
# -------------------------------------------------------
try:
...
import apps.tools.maya.sub_assets_republish.config as config
...
class SubAssetsInfoDialog(QDialog):
    ...
    def __init__(self, entity_list=[], dcc='maya'):
    ...
    def init_data(self):
        ...
        self.tableWidgetAssetInfoList.setHorizontalHeaderItem(8, QTableWidgetItem("关联cts(动卡)"))
    ...
    def set_sub_asset_info(self):
        cut_in_text = u"<span style='font-size:28pt'>%s</span>" % " 资产类型:".decode("utf8")
        cut_duration_text = u"<span style='font-size:28pt'>%s</span>" % "   资产备注:".decode("utf8")
        cut_in = ""
        cut_duration = ""
        if self.entity_type == "Asset":
            entity = self.task_shotgun.get_asset_entity(self.project_name, self.entity_name)
            if entity:
                cut_in_data = entity['sg_asset_type']
        ...
        asset_info = cut_in_text + cut_in + cut_duration_text + cut_duration

        self.labelShotInfo.setText(asset_info)
...
if __name__ == '__main__':
    entity_list = [{
        ...
        'entity.Asset.sg_asset_type': 'body',
        'entity': {'type': 'Asset', 'id': 13809, 'code': 'FY_BODY'},
        'content': 'rbf',
        'start_date': None,
        'sg_status_list': 'rev',
        'est_in_mins': None,
        'step': {'type': 'Step', 'id': 253,'name': 'rbf'},
        'task_assignees': [],
        'type': 'Task',
        'id': 92639
    }]
    ...
    win = SubAssetsInfoDialog(entity_list,dcc='maya')  # create an window
