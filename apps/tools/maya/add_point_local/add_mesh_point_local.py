...
def add_meshs_point_local(meshs, start_num=0):
    ...
    for i in range(len(meshs)):
        grp, start_num = add_mesh_point_local(meshs[i], start_num)
        if grp:
            grps.append(grp)
    return grps
...
def add_mesh_point_local(mesh, start_num=0):
    ...
    grp = cmds.group(em=True, name=grp_name)
    cmds.parent(locals, grp)
    return grp, int(start_num + len(points))


def creat_point_local(point, num):
    local_name = 'local_{}'.format(num)
    count = 1
    while cmds.ls(local_name):
        local_name = 'local_{}'.format(num + count)
        if not cmds.ls(local_name):
            break
        else:
            count += 1
