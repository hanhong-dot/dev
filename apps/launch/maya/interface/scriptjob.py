...
def close_port():
    ...
    mel.eval('optionVar -iv commandportOpenByDefault false;')
    # try:
...
def sceneopened():
    ...
    cmds.scriptJob(event=('PostSceneRead', close_port))
    cmds.scriptJob(event=('SceneOpened', close_port))
    cmds.scriptJob(event=('NewSceneOpened',close_port))
    cmds.scriptJob(event=('SceneOpened', delecte_scriptJob))
    cmds.scriptJob(event=('NewSceneOpened', delecte_scriptJob))

    # cmds.scriptJob(event=('SceneOpened', _set_textlistlu))
    #
...
def postsceneread():
    ...
    if _order:
        ...
        # 关闭cycle检查报警
        cmds.scriptJob(event=('PostSceneRead', 'cmds.cycleCheck(e=False)'))
        #  清理普天同庆病毒
        cmds.scriptJob(event=('PostSceneRead', _kill_job))
        # 设置vr2显示
        cmds.scriptJob(event=('SceneOpened', _set_DirectX11))

        cmds.scriptJob(event=('PostSceneRead', _set_gpu_override))
