...
def _load_launch():
    ...
    from apps.launch.maya.interface import maya_launch
    maya_version=_get_maya_version()
    if maya_version.startswith('2018'):
        maya_launch.launch("UI", module='py2')
    else:
        maya_launch.launch("UI", module='py3',version='2023')
...
def _load_maya_scriptjob():
    import sys
    sys.path.append('Z:/dev')
    from apps.launch.maya.interface import maya_launch
    maya_version = _get_maya_version()
    if maya_version.startswith('2018'):
        maya_launch.load_maya_scriptjob('2018')
    else:
        maya_launch.load_maya_scriptjob('2023')
...
cmds.evalDeferred("_load_launch()", lowestPriority=True)
cmds.evalDeferred("_load_maya_scriptjob()", lowestPriority=True)
cmds.scriptJob(event=('SceneOpened', _load_launch))
