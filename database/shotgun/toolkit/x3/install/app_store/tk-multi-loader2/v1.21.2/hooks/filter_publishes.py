...
class FilterPublishes(Hook):
    ...
    def execute(self, publishes, **kwargs):
        ...
        for publish in publishes:
            if publish["sg_publish"]["task"]:
                task_id = publish["sg_publish"]["task"]["id"]
                _task_status = app.shotgun.find_one("Task", [["id", "is", task_id]], ["sg_status_list"])
                if _task_status["sg_status_list"] == "omt":
