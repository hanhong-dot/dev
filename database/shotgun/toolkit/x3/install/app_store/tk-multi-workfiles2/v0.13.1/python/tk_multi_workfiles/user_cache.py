...
class UserCache(Threaded):
    ...
    def get_file_last_modified_user(self, path):
        ...
        if sgtk.util.is_windows():
            ...
            try:
                from apps.tools.maya import work_save
                reload(work_save)

                login_name = str(work_save.work_read_user(str(path)))
            except:
                pass

            # try:
            #     login_name = self._get_windows_ower_user(path)
            # except:
            #     login_name = self._get_windos_ower_user_by_cmd(path)
        else:
            try:
