...
ANIMBOTS = ['animBot', 'animBot_data']


def load_animbot():
    _animbot_server = '{}\\animBotTools'.format(TOOLSPATH)
    _animbot_server = _cover_z(_animbot_server)
    _animbot_server = _fix_path(_animbot_server)

    _animbot_local ='{}\\scripts'.format(_get_docs_path())

    #
    # 复制安装
    if not os.path.exists('{}\\{}'.format(_animbot_local, ANIMBOTS[1])):
        _result=_copy(_animbot_server, _animbot_local)
...
def _get_docs_path():
    ...
    if int(maya_version)< 2023:
        return os.path.expanduser(os.sep.join(["~", "maya"]))
    else:
        return os.path.expanduser(os.sep.join(["~", "Documents", "maya"]))

def _cover_z(_path):
    try:
        return _path.replace('Z:', '\\\\10.10.201.151\\share\\development')
    except:
        return _path.replace('z:', '\\\\10.10.201.151\\share\\development')
...
def _copy(source, targe):
    ...
    try:
        ...
        return False
    # os.system('xcopy {} {}'.format(source,targe))


def _process_copy(source, targe):
    if _is_admin():
        _copy(source, targe)
    else:
...
def _run_admin(cmd):
    ...
    :param: cmd: command requiring administrator premiere
    """
    cmd_bat = _get_bat()
    vbs_path = _get_vbs()
    try:
