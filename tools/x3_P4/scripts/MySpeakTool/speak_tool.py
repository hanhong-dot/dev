...
def get_config():
    config_path = os.path.abspath(__file__+"/../speak_config")
...
def load_npy(name):
    path = "D:/work/liuruomei/mengya/regression1/sequence_res/debug/%s.npy" % name
    cof = "D:/work/liuruomei/mengya/regression1/Config/blendshape_config.json"
    import json
    ...
    for i in range(ani_data.shape[0]):
        for name, index in bs_data["lip_sync"].items():
            ...
            pm.setKeyframe(bs, attribute=name, v=value, t=i)
...
def connect_bs():
    ...
    for name in src.listAttr(ud=1):
        ...
        if dst.attr(name).inputs():
            continue
        src.attr(name).connect(dst.attr(name))


def save_weights():
    sk = pm.PyNode("skinCluster2")
    ...
    import json
    with open("D:/work/liuruomei/data/mask.json", "w") as fp:
        json.dump(wts, fp)
