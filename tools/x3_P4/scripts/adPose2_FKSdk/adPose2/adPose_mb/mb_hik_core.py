...
def character_slots_link(pCharacter, slot, mbJoint):
    ...
    node = find_by_name(mbJoint)
    if mbJoint in ["Chest_M"]:
        names = ["Spine1_M", "Spine2_M", "Spine3_M"]
        index = len([n for n in names if find_by_name(n)])
        chest_slot = ["Spine", "Spine1", "Spine2", "Spine3"][index]
        if node:
            character.PropertyList.Find(chest_slot + "Link").append(node)
        return
    if not node:
        if mbJoint.startswith("MB_Control_"):
            node = find_by_name(mbJoint[11:])
            if node:
                character.PropertyList.Find(slot + "Link").append(node)
...
def check_hik_joints():
    names = ["Roots", "Root_M", "Spine1_M", "Head_M", "Scapula_L", "Scapula_R", "Neck_M",
                  "Chest_M", "Toes_L", "Toes_R"]
    names += ["Hip_L", "Knee_L", "Ankle_L",
                  "Hip_R", "Knee_R", "Ankle_R",
                  "Shoulder_L", "Elbow_L", "Wrist_L",
                  "Shoulder_R", "Elbow_R", "Wrist_R"]
    nodes = [find_by_name(name) for name in names]
    # loss_names = [n for n in names if not find_by_name(n)]
    return all(nodes)


def main():
    if not check_hik_joints():
        return
