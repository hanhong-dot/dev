...
def find_bs_ctrls(bs_nodes):
    ctrls = []
    for bs in bs_nodes:
        for target_name in cmds.listAttr(bs+".weight", m=1):
            attr = bs+"."+target_name
            ctrl = cmds.listConnections(attr, s=1, d=0)
            ctrls.extend(cmds.ls(ctrl, type="transform"))
            uu = cmds.ls(ctrl, type="animCurveUU")
            for u in uu:
                ctrl = cmds.listConnections(u, s=1, d=0)
                ctrls.extend(cmds.ls(ctrl, type="transform"))
    ctrls = list(set(ctrls))
    ctrls = cmds.ls(ctrls, type="transform")
    ctrls = [ctrl for ctrl in ctrls if is_shape(ctrl, Shape.nurbsCurve)]
    if "speakControl" in ctrls:
        ctrls.remove("speakControl")
    return ctrls
...
def doit():
    ...
    export_q_face_rig("D:/work/x3_face_rig_export/q_st/ry_face_v6.json")
